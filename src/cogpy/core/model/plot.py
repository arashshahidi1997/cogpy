...
def visualize_modes(modes, combinations, freq_to_view, time_to_view):
    ...
    fig, axes = plt.subplots(grid_dim, grid_dim, figsize=(15, 15), sharex=True, sharey=True)
    axes = axes.ravel()
    
    for idx, (ax, bump) in enumerate(zip(axes, modes)):
        loc = combinations[idx]
        bump.sel(frequency=freq_to_view, time=time_to_view).plot(ax=ax, cmap="viridis", vmin=canonical_min, vmax=canonical_max, add_colorbar=False)
