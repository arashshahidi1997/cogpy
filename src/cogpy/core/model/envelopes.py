...
def finite_duration_biexp(t, A=1.0, alpha=1.0, beta=0.5, t0=0.0, T=1.0,
                          window="smoothstep", renorm=None, **window_kwargs):
    """
    ...
    if renorm is not None:
        if renorm == "area":
            ...
            y_at_peak = (biexp(np.array([t_peak]), A=A, alpha=alpha, beta=beta, t0=t0)[0] *
                         (smoothstep_cutoff(np.array([t_peak]), t0, T)[0]
                          if window == "smoothstep" else
                          tukey_cutoff(np.array([t_peak]), t0, T, **window_kwargs)[0]))
            if y_at_peak != 0:
...
def example_usage():
    ...
    y_finite = finite_duration_biexp(t, A=1.0, alpha=1.5, beta=0.3, t0=0.5, T=2.0,
                                    window="smoothstep", renorm="area")

    # Or with a Tukey window (50% taper)
    y_tukey = finite_duration_biexp(t, A=1.0, alpha=1.5, beta=0.3, t0=0.5, T=2.0,
                                    window="tukey")
