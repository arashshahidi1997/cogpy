...
def compute_csd(dsigx):
    ...
    csdx = xr.DataArray(csddata, dims=('ch', 'time'), coords=dict(
            ch=dsigx.ch.values[1:-1],
            time=dsigx.time
        ))
    return csdx.transpose(*dsigx.dims)

def test_compute_csd():
    dsigx = xr.DataArray(np.random.randn(100, 16), dims=('time', 'ch'), coords=dict(time=np.arange(100), ch=np.arange(16)))
...
def preprocess_depth_sig(dsigx, median_kernel):
    ...
    dsigx_sm = xut.xarr_wrap(lambda x: nd.gaussian_filter(x, sigma=median_kernel))(dsigx_sm)
    return dsigx_sm
...
def get_max_df(x, time, ch, **kwargs):
    ...
    pa = x[pch, pit]
    pt = time[pit]
    max_df = pd.DataFrame(np.array([pt, pit, pch+1, pa]).T, columns=['tpeak', 'ipeak', 'ch', 'amp'])
    return max_df
