...
def classify_detections(
    detection_df: pd.DataFrame, ground_truth_df: pd.DataFrame, ordered_cols: list
):
    """
    ...
    true_positives = pd.merge(
        detection_df, ground_truth_df, on=ordered_cols, how="inner"
    )

    # False Positives: detection not in ground truth
    false_positives = detection_df.merge(
        ground_truth_df, on=ordered_cols, how="left", indicator=True
    )
    false_positives = false_positives[false_positives["_merge"] == "left_only"].drop(
        columns=["_merge"]
    )

    # False Negatives: ground truth not detected
    false_negatives = ground_truth_df.merge(
        detection_df, on=ordered_cols, how="left", indicator=True
    )
    false_negatives = false_negatives[false_negatives["_merge"] == "left_only"].drop(
        columns=["_merge"]
    )
