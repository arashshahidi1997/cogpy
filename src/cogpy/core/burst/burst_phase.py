...
def bandpass_filter(burst_sig, burst, fs, freq_halfbandwidth=5):
    ...
    burst_sig_bp = signal.filtfilt(
        *signal.butter(2, freq_band, "bandpass", fs=fs), burst_sig.data
    )
    burst_sig_bp = xr.DataArray(
        burst_sig_bp, coords=burst_sig.coords, dims=burst_sig.dims
    )
    return burst_sig_bp
...
def hilbert_transform(burst_sig_bp):
    ...
    burst_analytic = xr.DataArray(
        burst_sig_hilb, coords=burst_sig_bp.coords, dims=burst_sig_bp.dims
    )
...
def get_burst_sig_phase_and_amp(csig, burst, time_halfwindow=1, freq_halfbandwidth=5):
    ...
    burst_sig_bp = bandpass_filter(
        burst_sig, burst, fs=csig.fs, freq_halfbandwidth=freq_halfbandwidth
    )
...
def get_rel_time(burst, burst_analytic_sig):
    return burst_analytic_sig.assign_coords(
        {"time": burst_analytic_sig.time - burst.time}
    )
