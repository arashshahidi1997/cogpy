"""
Time alignment utilities for multi-rate data.

These helpers provide lightweight primitives for synchronizing multiple time
bases (e.g., LFP at 1 kHz and spectrogram at 20 Hz).
"""
...
def align_to_common_timebase(
    ...
    ----------
    times
        List of 1D time arrays from different modalities.
    method
        Alignment method: 'nearest', 'union', 'intersection'.
...
def find_nearest_time_index(target_time: float, time_array: np.ndarray) -> int:
    """
    Find index of nearest time in a sorted array.
    """
