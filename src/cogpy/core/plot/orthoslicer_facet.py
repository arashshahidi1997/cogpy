...
class OrthoSlicer(param.Parameterized):
    ...
    def __init__(self, array: xr.DataArray, dt=None, dz=None, dy=None, dx=None, clim_quantile=True, **params):
        ...
        if array.name is None:
            array.name = "value"
        self.vdim = hv.Dimension(array.name, label=array.name, unit=array.attrs.get('units', ''))
    ...
    def local_coord_mesh(self, mesh_shape=(4, 4, 4, 4)):
        ...
        for i, dim in enumerate(dims):
            ...
            coords[dim] = spaced_sample_around_coord(self.array, dim=dim, center=center, nsample=nsample)
    ...
    def get_facet_indices(self):
        ...
        tsamples = spaced_sample_around_coord(self.array, dim='t', center=self.t, nsample=4)

        # select regular sampling around frequency
        fsamples = spaced_sample_around_coord(self.array, dim='z', center=self.z, nsample=4)
    ...
    def prepare_meta(self):
        ...
        self._labels = {std_name: hvdim.label for std_name, hvdim in self.hvdims.items()}

        # make it explicit with attributes
        self.lx, self.ly, self.lz, self.lt = self._labels["x"], self._labels["y"], self._labels["z"], self._labels["t"]
        self.ox, self.oy, self.oz, self.ot = self._original_name["x"], self._original_name["y"], self._original_name["z"], self._original_name["t"]
        self.ux, self.uy, self.uz, self.ut = self._units["x"], self._units["y"], self._units["z"], self._units["t"]

    def standardize_coords(self):
        self.array = self.array.rename(self._rename_map)   # e.g. {'time':'t','freq':'z','ml':'x','ap':'y'}
    ...
    def get_bounds(self):
        b = {dim: (self.array[dim].min().item(), self.array[dim].max().item()) for dim in self.array.dims}
    ...
    def set_contrast_limits(self):
        ...
        if self.clim_quantile:
            clim_init = (float(np.percentile(self.array.values, 2)), float(np.percentile(self.array.values, 98)))
        else:
    ...
    def display_controls(self):
        return pn.Column(
            ...
            pn.Row(
                pn.widgets.Button(name="Reset XY Zoom", button_type="primary").param.watch(
                    lambda e: self._reset_xy_zoom(), "clicks"
                ),
                pn.widgets.Button(name="Reset TZ Zoom", button_type="primary").param.watch(
                    lambda e: self._reset_tz_zoom(), "clicks"
                )
            )
        )
    ...
    def panel_app(self):
        ...
        Right: XY orthoslice
        """
        tz_pane  = pn.pane.HoloViews(self.view_tz, width=800, height=420, sizing_mode="fixed")
        xy_pane  = pn.pane.HoloViews(self.view_xy, width=500, height=500, sizing_mode="fixed")
