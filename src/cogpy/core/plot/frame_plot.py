...
class FramePlot:
    ...
    @plot_context
    def widget_array(self, plot_func, interval=300, s=4, title=None, autoplay=True, **plot_kwargs):
        widget_array(self._arr, plot_func, interval=interval, s=s, title=title, autoplay=autoplay, **plot_kwargs)
...
def widget_array(arr_list, plot_func, interval=300, s=4, title=None, autoplay=True, **kwargs):
    ...
    if autoplay:
        slider = widgets.Play(min=0, max=arr_list[0][0].dur - 1, step=1, interval=interval)
...
extrema_button = widgets.ToggleButton(
    value=True,
    description='ext',
    icon='check'
)
...
contour_button = widgets.ToggleButton(
    value=True,
    description='contour',
    icon='check'
)
