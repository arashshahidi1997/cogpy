...
class erpPCA(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X, y=None):
        erppca_dict = erppca(X, nFac=self.nfac, maxIt=self.max_it, Tol=self.tol, IfVerbose=self.verbose, return_attrs=True)
...
def simplicity_criterion(Y):
    ...
    G1 = np.sum(B) - np.sum(np.diag(B)) # (1) , sum of off-diagonal elements: how orthogonal the columns are - for fully orthogonal Y**2 columns the off diagonal sum would be zero
...
def varimax_iter(Yo, Y):
    L, D, M = np.linalg.svd(Yo.T @ (Y.shape[0]*Y**3 - Y @ np.diag(np.sum(Y**2, axis=0))))
    T = L @ M # no transpose
    D = np.sum(D)
    Y = Yo @ T
    return Y, D
...
def similarity_metric(ss1: SpatSpecDecomposition, ss2: SpatSpecDecomposition, fac1, fac2, freq_threshold=5):
    ...
    correlation_val = np.corrcoef(ss1.ldx_slc_maxfreq[fac1].data.flatten(), ss2.ldx_slc_maxfreq[fac2].data.flatten())[0, 1]
    similarity = correlation_val
    return similarity

def compute_similarity_matrix(ss1: SpatSpecDecomposition, ss2: SpatSpecDecomposition, freq_threshold=5):
    ...
    for fac1 in range(nfac):
        for fac2 in range(nfac):
            similarity_matrix[fac1, fac2] = similarity_metric(ss1, ss2, fac1, fac2, freq_threshold)
...
def erpPCA_ldx(LR, ld_coords, ld_shape):
    ...
    nFac = LR.shape[-1]
    coords_ = ld_coords
    factors_ldx = xr.DataArray((LR.T).reshape(-1, *ld_shape),
