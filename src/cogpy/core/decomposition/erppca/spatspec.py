...
class DesignMatrixReshaper:
    ...
    def unstack_to_original(self, stacked_data):
        ...
        if self.original_dims is None:
            raise ValueError(
                "Original dimensions have not been set. Ensure 'stack_except' is called before 'unstack_to_original'."
            )
...
class SpatSpecDecomposition:
    def __init__(self, spatspec_coords, ld=None):
        ...
        if isinstance(spatspec_coords, xr.DataArray):
            print(
                "Warning: spatspec_coords is xr.DataArray, assuming spatspec_coords is spatspec_mtx"
            )
    ...
    def scx_process(self, scx, return_all=False, sigma=0.25, quantile=0.25):
        ...
        print("\t thresholding ...")
        scx_thresh = ts.threshold(
            scx_noenv_dict["scx_noenv"], quantile=quantile, axis=0
        )
    ...
    def scx_spikes(self, scx, max_halfdur=5.0, return_trace=False):
        fs = scx.fs
        ...
        for ifactor in range(nfactor):
            ...
            peripeak_wins = scx_peak_time.reshape(-1, 1) + periwin.reshape(
                1, -1
            )  # (npeak, nwin)
            ...
            fppx = np.array(
                [scx_peak_time, scx_peak_amp, scx_halfamp_dur]
            )  # feature, peak
    ...
    def plot_ldx_maxfreq(self):
        fig, axes = plot_maxfreq_slice_loadings(
            self.ldx_slc_maxfreq,
            self.ldx_df,
            nrow=self.nFac // 4,
            ncol=4,
            figsize=(4 * 3, self.nFac // 4 * 3),
        )
        return fig, axes
    ...
    def mark_highgamma_freq(self):
        ...
        self.ldx_df.loc[:, "is_highgamma_freq"] = self.ldx_df.factor.isin(
            highgamma_facs
        )
    ...
    def similarity_metric(self, other, fac1, fac2, freq_threshold=5):
        ...
        freq_diff = abs(
            self.ldx_df.freqmax.iloc[fac1] - other.ldx_df.freqmax.iloc[fac2]
        )
        ...
        correlation_val = np.corrcoef(
            self.ldx_slc_maxfreq[fac1].data.flatten(),
            other.ldx_slc_maxfreq[fac2].data.flatten(),
        )[0, 1]
    ...
    def compute_similarity_matrix(self, other, freq_threshold=5):
        ...
        for fac1 in range(nfac):
            for fac2 in range(nfac):
                similarity_matrix[fac1, fac2] = self.similarity_metric(
                    other, fac1, fac2, freq_threshold
                )
