...
def butteer_worth_widget(fs):
    @widgets.interact(
        order=widgets.IntSlider(4, 0, 10),
        f0=widgets.FloatSlider(value=1, min=0.1, max=10),
        f1=widgets.FloatSlider(value=50, min=10, max=100),
    )
    def plot_filt(order, f0, f1):
        ...
        ax.set(
            title="filter", xlabel="Frequency [Hz]", ylabel="Amplitude", xlim=[0.1, 100]
        )
...
def nanpad(input, w):
    return np.pad(input, pad_width=w // 2, mode="constant", constant_values=(np.nan,))[
        1:17, 1:17, :
    ]
...
class TemporalLowpassButter(Filter):
    ...
    def _filt(self, A):
        b, a = signal.butter(
            self.order, self.f_cutoff, btype="lowpass", output="ba", fs=self.fs
        )
...
class TemporalBandpassButter(Filter):
    ...
    def _filt(self, A):
        b, a = signal.butter(
            self.order, self.f_band, btype="bandpass", output="ba", fs=self.fs
        )
        # descriptor = {'name':'temporal lowpass butter', 'params':f'f_cutoff={f_cutoff}, order={order}'}
        return signal.filtfilt(b, a, A, axis=-1)
