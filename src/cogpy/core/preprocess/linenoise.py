...
class LineNoiseEstimatorICA(TransformerMixin):
    ...
    def fit(self, X):
        ...
        self.ica = FastICA(
            self.ncomp, whiten="unit-variance", max_iter=1000, tol=0.001, w_init=w_init
        )
        print("Fitting ICA...")
        ...
        self._detect_and_set_linenoise_components(
            self.NW, self.slider_kwargs, self.zscore_threshold
        )
    ...
    def _set_init_from(self, init_ica):
        ...
        if K is None:
            raise RuntimeError(
                "Your scikit-learn version may not expose `whitening_`. "
    ...
    def _construct_linenoise_signal(self, lnoise_ic_scores):
        ...
        linenoise_sig = (
            np.dot(self.lnoise_ica_mixing, lnoise_ic_scores).T + self.ica.mean_
        )
        return linenoise_sig

    def _detect_and_set_linenoise_components(
        self, NW, slider_kwargs, zscore_threshold=1.5
    ):
        ...
        ic_scx = xr.DataArray(
            self.ic_scores,
            dims=["comp", "time"],
            coords={"time": np.arange(self.ic_scores.shape[1]) / self.fs},
        )
        # spectrogram of IC scores
        mtx_scores = sp.mtm_spectrogramx(ic_scx, **mtm_gsp_kwargs)
        # sum linenoise harmonics
        linenoise_power = xr.concat(
            [mtx_scores.sel(freq=slc) for slc in self.hslices], dim="freq"
        ).sum(dim="freq")
        linenoise_power = linenoise_power.sum(dim="time") / mtx_scores.sum(
            dim=("freq", "time")
        )
        zlinenoise = sts.zscore(linenoise_power)
        zlinenoise_df = (
            pd.Series(zlinenoise, name="noise_zpower")
            .reset_index()
            .sort_values("noise_zpower", ascending=False)
        )
        zlinenoise_df["is_linenoise"] = zlinenoise_df["noise_zpower"] > zscore_threshold

        self.linenoise_components = zlinenoise_df["index"][
            zlinenoise_df["is_linenoise"]
        ].values
...
def sliding_ICA(sigx: xr.DataArray, fs, segment_size, lnoiseICA_params):
    ...
    sigx_coarsen = sigx.coarsen({"time": segment_size}, boundary="pad").construct(
        time=("segment", "time")
    )
    ...
    lnoise_estimator = LineNoiseEstimatorICA(
        fs, init_ica=lnoise_init.ica, **lnoiseICA_params
    )
    ...
    # repatch segments
    lnoise_estimate = xr.concat(
        [
            lnoise_estimate.isel(segment=i)
            for i in range(lnoise_estimate.sizes["segment"])
        ],
        dim="time",
    )
    ...
    assert np.allclose(
        np.isnan(lnoise_estimate.data), np.isnan(sigx.data)
    ), "NaN times of linenoise estimate and original signal do not match!"
