...
def resample_sigx(sigx: xr.DataArray, f_d=64, axis="time") -> xr.DataArray:
    ...
    if f_s is None:
        raise ValueError(
            "Input DataArray must have a 'fs' attribute specifying the sampling frequency."
        )
    ...
    resampled_coords = np.linspace(
        original_coords[0], original_coords[-1], resampled_array.shape[axis_num]
    )
