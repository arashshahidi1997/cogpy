...
def noise_to_signal(arr, fs, axis=-1, low_freq=(0.1, 30), high_freq=(30, 80)):
	...
	psd = xr.DataArray(psd, dims=dims_dof + ['freq'], coords={'freq': np.fft.rfftfreq(nperseg, 1/fs)})
...
def time_derivative(arr, axis=-1):
	...
	"""
	tder_ch = np.nanmean(
			np.abs(np.array(np.gradient(arr, axis=axis))),
		axis=axis)
	return tder_ch
...
def laplacian(arr, gridshape, stencil='9'):
	...
	a_laplacian = nd.convolve(arr, np.expand_dims(laplacian_stencil, axis=-1), mode='mirror')
	return a_laplacian
...
def spatial_gradient(arr):
	...
	rel_grad = np.nanmean(
		np.linalg.norm(
				np.array(np.gradient(arr, axis=(-3, -2)))
			, axis=-4),
		axis=-1)
	return rel_grad
...
def gradient(arr, adj=None):
	...
	med_grad = np.nanmedian(np.where(np.expand_dims(adj, axis=-1), np.abs(a_flat[None, :, :] - a_flat[:, None, :]), np.nan), axis=1)
	return np.nanmean(med_grad, axis=-1).reshape(grid_shape) # temporal_mean_neighbor_med_grad_ch
...
def gradient_original(arr, gridshape, loc_exclude_df):
	...
	grad = np.abs(arr[:, loc_exclude_df['ch_neighbor']] - arr[:, loc_exclude_df['ch_ref']])
	med_grad = np.array([np.nanmedian(grad[:, loc_exclude_df['ch_ref']==ch], axis=1) for ch in range(nchan)])
