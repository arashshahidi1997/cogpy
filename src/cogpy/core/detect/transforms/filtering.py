...
class HighpassTransform(Transform):
    """Highpass filter using `cogpy.core.preprocess.filtx.highpassx`."""
    ...
    def compute(self, data: xr.DataArray) -> xr.DataArray:
        from cogpy.core.preprocess.filtx import highpassx

        return highpassx(data, self.cutoff, self.order, axis=self.axis)


class LowpassTransform(Transform):
    """Lowpass filter using `cogpy.core.preprocess.filtx.lowpassx`."""
    ...
    def compute(self, data: xr.DataArray) -> xr.DataArray:
        from cogpy.core.preprocess.filtx import lowpassx
