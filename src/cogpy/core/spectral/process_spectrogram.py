...
def median_spec(mtx, med_size):
    ...
    mtx_filt.attrs['_filt_log'].append({'name': 'spatial lowpass median', 'params': {'size':med_size}})
    return mtx_filt
...
def gaussian_spec(mtx, gauss_sigma):
    ...
    if '_filt_log' not in mtx.attrs:
        mtx.attrs |= dict(_filt_log=[])
    mtx_filt.attrs = mtx.attrs.copy()
    mtx_filt.attrs['_filt_log'].append({'name': 'spatio-spectro-temporal lowpass gaussian', 'params': {'sigma':gauss_sigma}})
    return mtx_filt
...
def nan_if_outlier_generic(arr, kernel, threshold=2):
    ...
    """
    arr_filt = nd.generic_filter(arr, lambda x: nan_if_outlier(x, threshold), size=kernel, mode='reflect')
    return arr_filt
...
def fix_outliers(x, kernel, threshold=2):
    ...
    x_filt = nd.generic_filter(x, lambda x: nan_if_outlier(x, threshold), size=kernel, mode='reflect')
    # interpolate nans
