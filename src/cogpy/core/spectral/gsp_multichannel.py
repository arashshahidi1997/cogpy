...
__all__ = ["get_tapers", "mtm_spectrum", "mtm_spectrogram"]
...
def mtm_spectrum(
    ...
    *,
    ...
    tapers, lambdas = get_tapers(
        N, bandwidth, fs=fs, n_tapers=n_tapers, min_lambda=min_lambda
    )
...
def mtm_spectrogram(
    ...
    *,
    ...
    tapers, lambdas = get_tapers(
        nperseg, bandwidth, fs=fs, n_tapers=n_tapers, min_lambda=min_lambda
    )
    n_tapers = tapers.shape[0]
    ...
    data_strided = as_strided(data, shape=shape, strides=strides, writeable=False)
    ...
    out_timestamps = np.mean(
        as_strided(ts, shape=ts_shape, strides=ts_strides, writeable=False), axis=1
    )
    ...
    spectrogram = np.sum(lambdas[:, None, None] * spectrograms, axis=0) / np.sum(
        lambdas
    )
...
def mtm_spectrogram_multichannel(
    data,
    bandwidth,
    *,
    fs=1,
    ...
    Parameters
    ----------
    data : np.ndarray, with shape (C, T)
        Input data
    bandwidth : float
        Bandwidth of taper, in Hz
    fs : float, optional
    ...
    if remove_mean:
        data = data - data.mean(axis=time_axis)
