...
def dpss_tapers(N: int, NW: float = 2, K_max: int = None) -> np.ndarray:
	...
	if K_max is None:
		K_max = int(2 * NW - 1)
	assert (K_max > 0 and K_max < N / 2), print('increase resolution `N` or decrease time-frequency-half-bandwidth `NW`')
...
def multitaper_fft(y, axis=-1, NW=2, nfft=None, K_max=None, detrend=True, tapers=None):
	...
	shape_origin = tapered_y.shape[1:] # original shape without sample_dim (*dof, taper_dim)
	tapered_y = tapered_y.reshape(tapered_y.shape[0], -1) # shape (sample_dim, *dof * taper_dim)
...
def multitaper_psd(y, axis=-1, NW=2, nfft=None, K_max=None, detrend=True):
	...
	f: freq array
	"""
	mtfft_ = multitaper_fft(y, axis=axis, NW=NW, nfft=nfft, K_max=K_max, detrend=detrend)
	if nfft is None:
		nfft = y.shape[axis]
...
def specx_coords(specx, nfft, fs, freq_dim='freq', time_dim='time'):
	reorder_dims = [d for d in specx.dims if d not in (time_dim, freq_dim)] + [freq_dim, time_dim]
...
def multitaper_spectrogram(xsig: xr.DataArray, NW=2, window_size: int = 256, window_step: int = 64, fs=None, nfft=None, detrend=True, time_dim='time', window_dim='window', freq_dim='freq'):    
	mtx = running_spectral(
		multitaper_psd, xsig,
		measure_kwargs=dict(NW=NW, detrend=detrend),
		window_size=window_size, window_step=window_step,
		fs=fs, nfft=nfft,
		time_dim=time_dim, window_dim=window_dim, freq_dim=freq_dim, extra_output_dims=[],
		extra_output_sizes={},
		output_dtype=float
	)
	return mtx

def multitaper_fftgram(xsig: xr.DataArray, NW=2, window_size: int = 256, window_step: int = 64, fs=None, nfft=None, detrend=True, time_dim='time', window_dim='window', freq_dim='freq', taper_dim='taper'):
	ntapers = 2*NW-1
	mtx_fft = running_spectral(
		...
		window_size=window_size, window_step=window_step,
		fs=fs, nfft=nfft,
		time_dim=time_dim, window_dim=window_dim, freq_dim=freq_dim, extra_output_dims=[taper_dim],
		extra_output_sizes={taper_dim: ntapers},
		output_dtype=np.complex128
	)
	mtx_fft = mtx_fft.assign_coords({taper_dim: np.arange(ntapers)+1})
	return mtx_fft.transpose(*[d for d in mtx_fft.dims if d not in (time_dim, freq_dim, taper_dim)], time_dim, taper_dim, freq_dim)

def running_spectral(
		spectral_measure: Callable,
		xsig: xr.DataArray,
		measure_kwargs: Dict[str, Any],
		window_size: int = 256, window_step: int = 64,
		fs=None, nfft=None,
		time_dim='time', window_dim='window', freq_dim='freq',
	...
	assert time_dim in xsig.dims, f'{time_dim} not in xsig.dims, please pass the name of your time dimension to time_dim in kwargs'
	assert window_dim not in xsig.dims, f'{window_dim} already in xsig.dims, please pass a different name for window dimension to window_dim in kwargs'
	...
	assert isinstance(extra_output_dims, list), 'output_dims should be a list of strings with names of output dimensions other than frequency dimension'
	...
	assert isinstance(extra_output_sizes, dict), 'output_sizes should be a dictionary with keys as output dimension names and values as their sizes'
	output_sizes.update(extra_output_sizes)

	# check output sizes and dims
	for output_dim in output_dims:
		assert output_dim not in xsig.dims, f'{output_dim} already in xsig.dims, please pass a different name for frequency dimension to output_dim in kwargs'
		assert output_dim in output_sizes, f'except for frequency size, please pass the size of output dimension {output_dim} in output_sizes kwargs'
