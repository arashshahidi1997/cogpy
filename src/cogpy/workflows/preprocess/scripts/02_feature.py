...
def feature(
    sigx: xr.DataArray, window_size: int, window_step: int, zscore: bool
) -> xr.Dataset:
    AP = sigx.sizes["AP"]
    ML = sigx.sizes["ML"]
    fs = sigx.fs
    ...
    return chfeat.compute_features(
        sigx, window_size=window_size, window_step=window_step, zscore=zscore
    )
