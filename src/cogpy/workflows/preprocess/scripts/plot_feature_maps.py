...
def feature_df(feature_ds, outlier_map):
	...
	df_bad = xr.DataArray(outlier_map.reshape(AP, ML), dims=['AP', 'ML']).to_dataframe(name='is_bad')
...
def bad_score_map(feature_ds):
	...
	qfeat_stacked = qfeat_arr.stack(qfeat=['feature', 'quantile']).transpose('ch', 'qfeat')
...
def plot_feature_maps(feature_ds, outlier_map):
	...
	points_bad_chs = hv.Points(bad_chs_df, ['ML', 'AP']).opts(opts.Points(color='red', s=12))
	bad_image = hv.Image(badxmap, ['ML', 'AP']).opts(title="Aggregate: Bad Score", fontsize={'title': 16})

	all_images = [hv.Image(ds, ['ML', 'AP'], vdims=feature_name)\
				.opts(title=feature_name.replace("_", " ")) for feature_name in feature_maps.data_vars] \
		+ [bad_image]
