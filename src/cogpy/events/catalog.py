...
class EventCatalog:
    ...
    def __post_init__(self) -> None:
        if not isinstance(self.df, pd.DataFrame):
            raise TypeError(
                f"EventCatalog.df must be a pandas DataFrame, got {type(self.df).__name__}"
            )
        ...
        t = pd.to_numeric(self.df["t"], errors="coerce").to_numpy(
        ...
        if ("t0" in self.df.columns) and ("t1" in self.df.columns):
            t0 = pd.to_numeric(self.df["t0"], errors="coerce").to_numpy(
                dtype=float, copy=False
            )
            t1 = pd.to_numeric(self.df["t1"], errors="coerce").to_numpy(
                dtype=float, copy=False
            )

            if not (np.isfinite(t0).all() and np.isfinite(t1).all()):
                raise ValueError(
                    "Interval columns 't0'/'t1' must be finite when present"
                )
            if not np.all(t1 > t0):
    ...
    def to_events(self):
        ...
        times = pd.to_numeric(self.df["t"], errors="coerce").to_numpy(
            dtype=float, copy=False
        )
        labels = (
            self.df["label"].to_numpy(dtype=str, copy=False)
    ...
    def to_intervals(self):
        ...
        starts = pd.to_numeric(self.df["t0"], errors="coerce").to_numpy(
            dtype=float, copy=False
        )
        ends = pd.to_numeric(self.df["t1"], errors="coerce").to_numpy(
            dtype=float, copy=False
        )
    ...
    def to_event_stream(self, style: Any | None = None):
        ...
        if style is None:
            ...
            raise TypeError(
                "style must be None, an EventStyle, or a dict of EventStyle fields"
            )
    ...
    def from_hmaxima(
        ...
        **metadata: Any,
        ...
        df["t"] = pd.to_numeric(df[time_col], errors="coerce").to_numpy(
            dtype=float, copy=False
        )
        df["label"] = str(label)
        if value_col is not None:
            df["value"] = pd.to_numeric(df[value_col], errors="coerce").to_numpy(
                dtype=float, copy=False
            )
    ...
    def from_burst_dict(
        ...
        **metadata: Any,
        ...
        if (
            ...
            and ("bandwidth" not in df.columns)
        ):
            df["bandwidth"] = pd.to_numeric(df["f1"], errors="coerce") - pd.to_numeric(
                df["f0"], errors="coerce"
            )
    ...
    def filter_by_spatial(
        ...
        *,
        ...
        if (AP is None) and (ML is None):
            return EventCatalog(
                df=df.copy(), name=str(self.name), metadata=dict(self.metadata)
            )
        ...
        if (AP is not None) and (ML is not None) and (radius is not None):
            ...
            ap = pd.to_numeric(df["AP"], errors="coerce").to_numpy(
                dtype=float, copy=False
            )
            ml = pd.to_numeric(df["ML"], errors="coerce").to_numpy(
                dtype=float, copy=False
            )
