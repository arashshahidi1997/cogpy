...
def compute_correlation_matrix(burst_analytic_sig):
    burst_sig_data = np.concatenate([burst_sig_.data[None, :] for burst_sig_ in burst_analytic_sig], axis=0)
    return np.corrcoef(np.real(burst_sig_data))

def compute_distances(burst_df):
    # Temporal distance
    burst_time_diff = np.abs(burst_df.time.values[:, None] - burst_df.time.values[None, :])
...
def assign_quadrants(burst_df):
    ...
    burst_df.loc[:, 'quadrant'] = burst_df.apply(lambda x: quadrant_names[(np.sign(x.AP), np.sign(x.ML))], axis=1)
    return burst_df

def create_corr_df(burst_corr, burst_time_diff, burst_spatial_dist, burst_df, time_thresh=DEFAULT_TIME_HALFWINDOW):
    ...
    is_colocalized = burst_df.quadrant.values[:, None] == burst_df.quadrant.values[None, :]
    colocalized_series = is_colocalized[mask].reshape(-1)
    burst_corr_df = pd.DataFrame(np.array([spat_series, time_series, corr_series, colocalized_series, concurrent_series
                                        ]).T, columns=['spat_dist', 'time_dist', 'corr_val', 'colocalized', 'concurrent'])
...
def filter_pairs_for_merging(burst_corr_df, corr_thresh=0.9):
    ...
    burst_corr_merge_df = burst_corr_df[(burst_corr_df.corr_val >= corr_thresh) & (burst_corr_df.colocalized) & (burst_corr_df.concurrent)]
...
def find_merge_clusters(burst_corr_merge_df):
    ...
    burst_merge_clusters_list = [list(cluster) for cluster in list(nx.connected_components(G_merge))]
...
def merge_duplicate_bursts(burst_df, csig, time_thresh=0.5, freq_halfbandwidth=2.5, time_halfwindow=DEFAULT_TIME_HALFWINDOW, corr_thresh=0.9, verbose=False):
    burst_analytic_sig = get_burst_analytic(burst_df, csig, freq_halfbandwidth=freq_halfbandwidth, time_halfwindow=time_halfwindow)
    ...
    burst_corr_df = create_corr_df(burst_corr, burst_time_diff, burst_spatial_dist, burst_df, time_thresh=time_thresh)
    burst_corr_merge_df = filter_pairs_for_merging(burst_corr_df, corr_thresh=corr_thresh)
