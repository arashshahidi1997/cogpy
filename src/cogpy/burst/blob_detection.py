...
def detect_hmaxima(datax: xr.DataArray, h_quantile: float = 0.8, h=None, footprint=None) -> pd.DataFrame:
    """
...
def get_coo_df(datax, h_maxima):
    ...
    bursts_icoos_df = pd.DataFrame({dim: bursts_icoos[dim_idx] for dim_idx, dim in enumerate(datax.dims)}, 
                                   columns=datax.dims)
    ...
    if hasattr(datax, 'indexes') and all(dim in datax.indexes for dim in datax.dims):
        for dim in datax.dims:
            bursts_icoos_df[f'i{dim}'] = bursts_icoos_df.apply(lambda row: datax.indexes[dim].get_loc(row[f'{dim}']), axis=1)
...
def set_sigma_dict(sigma_dict, datax, return_tuple=True):
    ...
    min_sigma_dict = {dim: int(val*fs_dict[dim]) for dim, val in min_sigma_dict.items()}
    max_sigma_dict = {dim: int(val*fs_dict[dim]) for dim, val in max_sigma_dict.items()}
...
def detect_blobs(datax: xr.DataArray, num_sigma:int=10, sigma_dict_raw=None) -> pd.DataFrame:
    """
    ...
    blobs_doh = blob_log(datax.data, min_sigma=min_sigma, max_sigma=max_sigma, num_sigma=num_sigma, threshold_rel=.2, exclude_border=1)
    coords = datax.coords
    dims = datax.dims
    blob_df = pd.DataFrame(blobs_doh, columns=['i' + dim for dim in dims] + ['i'+dim+'_sigma' for dim in dims])
    ...
    # add amplitude
    blob_df.loc[:, 'amp'] = blob_df[idims].apply(lambda x: datax.isel(**{dim: x[idim] for idim, dim in zip(idims, dims)}).values, axis=1)
    return blob_df
