...
class GaussianMode(BaseMode):
    ...
    def generate_mode(self, cov_matrix=None, sigma=None):
        ...
        if sigma is not None:
            ...
            if sigma.shape != (len(self.shape),):
                raise ValueError(
                    "Sigma must have the same number of dimensions as the mode."
                )
    ...
    def shift_mode(self, mode: xr.DataArray, loc=None):
        ...
        if self.boundary_condition == "periodic":
            ...
            shifted_mode = mode.roll(
                **{dim_name: shift for dim_name, shift in zip(mode.dims, shifts)}
            )

        else:  # non-periodic
            shifted_mode = mode.shift(
                **{dim_name: locus for dim_name, locus in zip(mode.dims, loc)},
                fill_value=0,
            )
        return shifted_mode
...
class GaussianCover(BaseCover):
    ...
    def __init__(
        ...
        sigma=None,
        spacing=None,
        boundary_condition="non-periodic",
    ):
        """
        ...
        if spacing is None:
            ...
            assert all(
                s > 0 for s in self.spacing
            ), "Spacing must be positive in all dimensions."
            assert len(self.spacing) == len(
                shape
            ), "Spacing must have the same number of dimensions as shape."

        self.gaussian_mode = GaussianMode(
            shape, boundary_condition=self.boundary_condition
        )
        self.canonical_mode = self.gaussian_mode.generate_mode(
            cov_matrix=self.cov_matrix, sigma=self.sigma
        )
        self.canonical_min = self.canonical_mode.min()
        self.canonical_max = self.canonical_mode.max()

        self.indices = [
            np.arange(-dim // 2 + s // 2, dim // 2, s) for dim, s in zip(shape, spacing)
        ]
        self.combinations = list(itertools.product(*self.indices))

        self.modes = [
            self.gaussian_mode.shift_mode(self.canonical_mode, loc)
            for loc in self.combinations
        ]
