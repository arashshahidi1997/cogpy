...
def _build_completion_parser(prog: str) -> argparse.ArgumentParser:
    ap = argparse.ArgumentParser(prog=prog, formatter_class=_HelpFormatter, add_help=False)
...
def main(argv=None):
	...
	ap.add_argument("input_spec",
					help="Input spec: path/to/file/filename.lfp")
	...
	# Config merging
	ap.add_argument("--configfile", type=Path,
	...
	with as_file(files("cogpy.workflows.preprocess") / "Snakefile") as snakefile_path, \
		...
		cmd = [
			...
			"--configfile", str(merged_cfg_path),
			"-d", str(workdir),
			"-c", str(args.cores),
		]
