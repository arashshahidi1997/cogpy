...
def from_file(dat_file: Union[str, Path], xml_file: str = None,  metadata: Dict[str, Any] = None, ML_coords: np.ndarray = None, AP_coords: np.ndarray = None) -> xr.DataArray:
	"""
	...
	data_array = xr.DataArray(dask_array, dims=('time', 'ML', 'AP'), coords={'time': time_coords, 'ML': ML_coords, 'AP': AP_coords})
...
def assert_ecog(sigx: xr.DataArray) -> None:
	...
	assert 'AP' in sigx.dims and 'ML' in sigx.dims and 'time' in sigx.dims, "sigx must have dimensions 'AP', 'ML', and 'time'."
	
def save_dat(arr: np.ndarray, dat_file: str, extension='.dat', dtype=np.int16, **save_kwargs):
	"""
