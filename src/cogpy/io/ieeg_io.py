...
def load_ieeg_metadata(lfp_path):
    ...
    return {
        ...
        "AP":     elec.groupby("row")["AP"].mean().reindex(range(meta["RowCount"])).to_numpy(),
        "ML":     elec.groupby("col")["ML"].mean().reindex(range(meta["ColumnCount"])).to_numpy(),
    }
...
def from_file(dat_file: Union[str, Path], grid=False, as_float=False) -> xr.DataArray:
	...
	if grid:
		...
		data_array = xr.DataArray(dask_array, dims=('time', 'ML', 'AP'), coords={'time': time_coords, 'ML': metadata['ML'], 'AP': metadata['AP']})
		...
		data_array = xr.DataArray(dask_array, dims=('time', 'ch'), coords={'time': time_coords, 'ch': channel_coords})
		data_array.attrs['fs'] = metadata['fs']
