...
def _cmd_presets() -> int:
    from cogpy.tensorscope.layout import LayoutManager
...
def _cmd_config(show: bool) -> int:
    if not show:
        return 0
    from cogpy.tensorscope import __version__
...
def _cmd_modules() -> int:
    from cogpy.tensorscope.modules import ModuleRegistry
...
def _cmd_serve(
    ...
    if module:
        if str(module) == "psd_explorer":
            from cogpy.tensorscope import TensorScopeApp
        ...
        from cogpy.tensorscope import TensorScopeState
        from cogpy.tensorscope.layers.controls import ProcessingControlsLayer
        from cogpy.tensorscope.modules import ModuleRegistry
