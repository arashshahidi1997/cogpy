...
def match_factors(spatspec_series, nrec, nfac, freq_threshold=3, simil_score_threshold=0.7):    
    match_df, opt_ref = get_fac_match_df(spatspec_series, freq_threshold, nrec, nfac, simil_score_threshold)
    ...
    return {
        ...
        'metadata_csv': pd.Series({'opt_ref': opt_ref, 'freq_threshold': freq_threshold, 'simil_score_threshold': simil_score_threshold})
    }
...
def get_remapping(simil_arr, nrec):
    ...
    for i in np.ndindex((nrec, nrec)):
        ...
        row_ind, col_ind = linear_sum_assignment(-simil_arr[i])  # Using negative as it finds minimum
...
def get_match_fac_ref(simil_arr, optimal_remapping, refrec, nrec, nfac, simil_score_threshold=0.7):
    """
    Parameters:
    ...
    match_fac_ref = pd.DataFrame(np.vstack(optimal_remapping[refrec])[1::2].T).sort_values(refrec)
    for i, j in np.ndindex((nrec, nrec)):
        match_fac_ref.loc[:, f's{i}_{j}'] = match_fac_ref.apply(lambda x: similx_flat.sel(r0=(i,x.loc[i]), r1=(j,x.loc[j])).item(), axis=1)

    match_fac_ref.loc[:, 'simil_arr'] = match_fac_ref.apply(lambda x: [[similx_flat.sel(r0=(i,x.loc[i]), r1=(j,x.loc[j])).item() for j in range(nrec)] for i in range(nrec)], axis=1)
    match_fac_ref.loc[:, 'simil'] = match_fac_ref.apply(lambda x: [similx_flat.sel(r0=(i,x.loc[i]), r1=(j,x.loc[j])).item() for i, j in itertools.combinations(range(nrec), 2)], axis=1)
    match_fac_ref.loc[:, 'simil_score'] = match_fac_ref.loc[:, 'simil'].apply(lambda x: min(x))
    match_fac_ref = match_fac_ref.sort_values('simil_score', ascending=False).reset_index(drop=True)

    match_fac_ref = match_fac_ref.reset_index().rename(columns={'index':'matched_factor'})
...
def optimal_refrec(match_fac_ref, eps: float=0.3):
    ...
    return np.argmax([match_metric(match_fac_ref[iref].simil_score, eps).mean() for iref in range(nrec)])
...
def get_fac_match_df(ss_series, freq_threshold, nrec, nfac, simil_score_threshold):
    ...
    match_fac_ref = [get_match_fac_ref(simil_arr, optimal_remapping, refrec, nrec, nfac, simil_score_threshold) for refrec in range(nrec)]
...
def concat_ss_series(ss_series):
    ...
    ss_concat.ldx_slc_maxfreq = xr.concat([ss.ldx_slc_maxfreq for ss in ss_list], dim='rec')
...
def plot_matched_facs(ss_match_cc, title, file):
    ...
    for irec in range(nrec):
        for ifac in range(nmatch_fac):
            axis = ax[ifac, irec]
            ss_match_cc.ldx_slc_maxfreq.sel(factor=ifac)[irec].plot.imshow(cmap='jet', vmin=0, ax=axis)
