...
def plot_maxfreq_slice_loadings(maxfreq_slice, factor_labels, nrow=4, ncol=4, figsize=(12,12)):
...
def plot_maxfreq_slc(ss, imshow_kwargs=None):
    ...
    for ifactor, ax in enumerate(axes.flatten()):
        ...
        ax.set(title=f'fac{ifactor}, {ss.ldx_df.loc[ifactor].freqmax:.0f}Hz', xlabel='ML', ylabel='AP')
