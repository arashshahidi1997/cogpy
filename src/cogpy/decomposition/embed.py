...
def _rolling_window(win, ref):
    ...
    ref: int
    """
    return np.arange(win, dtype=int).reshape(1, -1) + np.arange(
        ref - win + 1, dtype=int
    ).reshape(-1, 1)
...
def generic_kernel(x, kernel_shape):
    ...
    even_dim_index = np.arange(
        0, 2 * x.ndim, 2
    )  # original dims and sliding window dims alternate
    odd_dim_index = even_dim_index + 1
    x_win = x_win.transpose(
        *even_dim_index, *odd_dim_index
    )  # organizing dims such that shape=(original dims, sliding dims)
    return x_win
...
def embed_chan(x_embed, kernel_shape, bad_dims):
    ...
    if kernel_halfwidth:
        ...
        outside_win = nd.maximum_filter(
            outside_win, footprint=np.ones((kernel_grid, kernel_grid))
        )
