...
def running_corrcoef(xsig_flat: xr.DataArray, window_size: float, window_step: float, run_dim='time', corrcoef_dim='ch'):
    try:
        fs = xsig_flat.fs
    except AttributeError:
        raise ValueError("Input `xsig_flat` must have a `.fs` attribute indicating the sampling frequency.")

    assert corrcoef_dim in xsig_flat.dims, f"Dimension '{corrcoef_dim}' not found in input data."
    ...
    run_corrcoef = running_measure(
        np.corrcoef, xsig_flat, fs, 
        slider_kwargs=dict(window_size=seconds_to_samples(window_size, fs), window_step=seconds_to_samples(window_step, fs)), 
...
def select_distant_channel_pairs(
    ...
    coords = ch_stereotaxic_coords_df[['ml', 'ap', 'dv']].to_numpy(dtype=float)  # (N, 3)
...
def compute_emg_proxy(x_data, fs, coords_df, min_distance, window_size=0.5, window_step=0.25, return_corr_da=False):
    # bandpass the data
