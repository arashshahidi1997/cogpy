...
from .intervals import (check_intervals_disjoint, 
...
def label_numbers_by_state_intervals(tarr: np.ndarray, states: dict, progress_bar=True) -> pd.DataFrame:
    """
    Parameters
    ----------
    tarr : np.ndarray
    ...
    """
    tmap_df = {}
    iterator = tqdm(states.items(), desc='Processing States') if progress_bar else states.items()
...
def sort_col_into_states(df: pd.DataFrame, col: str, states: dict, progress_bar=True) -> pd.DataFrame:
    """
    ...
    if any(state in df.columns for state in states.keys()):
        warnings.warn('One or more state labels are already columns in the dataframe. The result will have duplicate column names.\
                        Consider changing the column names or the keys of the states by adding a prefix.')
    t_df = label_numbers_by_state_intervals(df[col].values, states, progress_bar=progress_bar)
    t_df.index = df.index
    df = pd.concat([df, t_df], axis=1)
    return df

def filter_by_states(df_labeled_states, include_states, exclude_states, return_complement=False):
    """
    ...
    """
    cnd_map = lambda row: all(row[state] == -1 for state in exclude_states) & any(row[state] != -1 for state in include_states) 
...
def get_exclusive_state_df(rec_df, col, state, states):
    ...
    rec_df_ = filter_by_states(rec_df_, include_states=[state], exclude_states=exclude_states)
    # drop excluded states
    rec_df_ = rec_df_.drop(columns=exclude_states)
    rec_df_ = rec_df_.sort_values(col).reset_index(drop=True).reset_index().rename(columns={'index': 'idx'}).set_index([state, 'idx'])
    return rec_df_
...
def state_transitions(states: dict, next_state_: bool = True) -> pd.DataFrame:
    ...
    for irow in range(n):
        ...
        if next_state_:
            edges.append({'transition_time': end_time, 'prev_state': current_state, 'next_state': next_state})
        else:
            edges.append({'transition_time': start_time, 'prev_state': prev_state, 'next_state': current_state})
...
def state_transition_interval(states, start_state, end_state, window_before, window_after):
    """
...
def append_transition_intervals(states, start_state, end_state, window_before=30, window_after=0):
    """
    ...
    transition_intervals = state_transition_interval(states, start_state, end_state, window_before, window_after)
...
def append_intermediate_sleep_intervals(states, IS_dur=30):
    ...
    IS_transitions = state_df[(state_df['prev_state'] == 'PerSWS') & (state_df['next_state'] == 'PerREM')]
    IS_intervals = [[t_end-IS_dur, t_end] for t_end in IS_transitions.transition_time.values]
...
def drop_corrupt_intervals(states: dict) -> dict:
    ...
    for state, intervals in states.items():
        intervals_clean = [interval for interval in intervals if interval[1] > interval[0]]
        states_clean[state] = intervals_clean
    return states_clean

def drop_micro_states(macro_state_intervals: np.ndarray, micro_state_intervals: np.ndarray) -> np.ndarray:
    """
    ...
    df_macro = sort_col_into_states(df_macro, 't_start', {'micro_start': micro_state_intervals}, progress_bar=False)
    df_macro = sort_col_into_states(df_macro, 't_end', {'micro_end': micro_state_intervals}, progress_bar=False)
    # find cases where both micro_start and micro_end are the same but not -1
    macro_within_micro = df_macro[(df_macro['micro_start'] != -1) & (df_macro['micro_end'] != -1) & (df_macro['micro_start'] == df_macro['micro_end'])]
    ...
    df_ = sort_col_into_states(df, 't_start', {'macro_start': macro_state_intervals}, progress_bar=False)
    df_micro = sort_col_into_states(df_, 't_end', {'macro_end': macro_state_intervals}, progress_bar=False)
    ...
    for macro_state_idx, macro_interval in enumerate(macro_state_intervals):
        ...
        start_micro = df_micro[(df_micro['macro_start'] == -1) & (df_micro['macro_end'] != -1)]
        if not start_micro.empty:
            macro_interval[0] = start_micro['t_end'].values[0]
        # get the row with macro_start != -1 and macro_end == -1, if there are any
        end_micro = df_micro[(df_micro['macro_start'] != -1) & (df_micro['macro_end'] == -1)]
        if not end_micro.empty:
            macro_interval[1] = end_micro['t_start'].values[0]
        # get the rows with macro_start != -1 and macro_end != -1
        df_micro_within = df_micro[(df_micro['macro_start'] != -1) & (df_micro['macro_end'] != -1)]
        ...
        split_macro_state_intervals = split_macro_state_intervals + macro_split_intervals
    return [list(interval_) for interval_ in split_macro_state_intervals]

def purify_macro_states(states: dict, macro_states:List[str], micro_states:List[str]) -> dict:
    """
    ...
    """
    assert all(macro_state in states for macro_state in macro_states), 'All macro states should be present in the states dictionary'
    assert all(micro_state in states for micro_state in micro_states), 'All micro states should be present in the states dictionary'
    assert len(set(macro_states).intersection(set(micro_states))) == 0, 'Macro states and micro states should be disjoint'
