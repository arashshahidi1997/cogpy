...
def mtm_spectrogram_multichannel(data, bandwidth, *, fs=1, timestamps=None, nperseg=None, noverlap=None,
                    n_tapers=None, min_lambda=0.95, remove_mean=False, nfft=None,
                    n_fft_threads=cpu_count()):
    ...
    step = nperseg - noverlap
    shape = data.shape[:-1]+((data.shape[-1]-noverlap)//step, nperseg) # (C, T, <nperseg>)
    strides = data.strides[:-1]+(step*data.strides[-1], data.strides[-1])
    data_strided = as_strided(
        data,
        shape=shape,
