...
def test_compute_emg_proxy():
    ...
    assert (
        "emg_proxy" in emg_proxy_df.columns
    ), "emg_proxy_df should contain 'emg_proxy' column"
    assert (
        "emg_proxy_std" in emg_proxy_df.columns
    ), "emg_proxy_df should contain 'emg_proxy_std' column"
    ...
    assert corr_da.dims == (
        ...
        "ch1",
        "ch2",
    ), "corr_da should have dimensions (time, ch1, ch2)"
