...
def test_map_numbers_to_intervals():
    ...
    assert (
        output_map == expectesd_map
    ), f"Mapping of numbers to intervals is incorrect. Expected: {expectesd_map}, Got: {output_map}"
...
def test_map_numbers_to_disjoint_intervals():
    ...
    assert np.array_equal(
        output_map, expected_map
    ), f"Mapping of numbers to disjoint intervals is incorrect. Expected: {expected_map}, Got: {output_map}"
...
def test_subtract_intervals():
    ...
    ), f"Subtraction of intervals is incorrect. Expected: {expected_output}, Got: {output}"
...
def test_get_state_durations():
    ...
    try:
        ...
        pytest.fail(
            f"Getting state durations is incorrect. Expected: {expected_output}, Got: {output}"
        )
...
def test_sort_col_into_states():
    ...
    try:
        ...
        pytest.fail(
            f"Sorting column into states is incorrect. Expected: {expected_df}, Got: {df_labeled_states}"
        )
...
def test_filter_by_states():
    ...
    df_state = filter_by_states(
        df, include_states=["PerSWS"], exclude_states=["PerREM"]
    )
    expected_df = pd.DataFrame(
        {"time": [15, 35], "PerSWS": [0, 1], "PerREM": [-1, -1]}, index=[1, 3]
    )
    try:
        pdt.assert_frame_equal(df_state, expected_df)
    except:
        pytest.fail(
            f"Filtering by states is incorrect. Expected: {expected_df}, Got: {df_state}"
        )
...
def test_get_exclusive_state_df():
    ...
    expected_df = (
        expected_df.sort_values("time")
        .reset_index(drop=True)
        .reset_index()
        .rename(columns={"index": "idx"})
        .set_index(["PerSWS", "idx"])
    )
    try:
        pdt.assert_frame_equal(rec_df_, expected_df)
    except:
        pytest.fail(
            f"Filtering by exclusive state is incorrect. Expected: {expected_df}, Got: {rec_df_}"
        )
...
def test_check_disjoint_states():
    ...
    expected_output = pd.DataFrame(
        [[True, True], [True, True]],
        index=["PerSWS", "PerREM"],
        columns=["PerSWS", "PerREM"],
    )
    try:
        pdt.assert_frame_equal(output, expected_output)
    except:
        pytest.fail(
            f"Checking disjoint states is incorrect. Expected: {expected_output}, Got: {output}"
        )
    ...
    expected_output = pd.DataFrame(
        ...
        index=["PerSWS", "PerREM", "PerIS"],
        columns=["PerSWS", "PerREM", "PerIS"],
    )
    try:
        pdt.assert_frame_equal(output, expected_output)
    except:
...
def test_state_transitions():
    ...
    try:
        ...
        pytest.fail(
            f"State transitions are incorrect. Expected: {expected_output}, Got: {output}"
        )
...
def test_state_transitions_interval():
    ...
    window_before = 5
    window_after = 5
    output = state_transition_interval(
...
def test_append_transition_intervals():
    states = {"PerSWS": [[10, 20], [30, 40]], "PerREM": [[20, 30], [40, 50]]}
    output = append_transition_intervals(
        states, "PerSWS", "PerREM", window_before=5, window_after=5
    )
    expected_output = {
        "PerSWS": [[10, 20], [30, 40]],
        "PerREM": [[20, 30], [40, 50]],
        "PerSWS_To_PerREM": [[15, 25], [35, 45]],
    }
    assert (
        output == expected_output
    ), f"Appending transition intervals is incorrect. Expected: {expected_output}, Got: {output}"
...
def test_append_intermediate_sleep_intervals():
    ...
    assert (
        output == expected_output
    ), f"Appending intermediate sleep intervals is incorrect. Expected: {expected_output}, Got: {output}"
...
def test_drop_corrupt_intervals():
    ...
    assert (
        output == expected_output
    ), f"Dropping corrupt intervals is incorrect. Expected: {expected_output}, Got: {output}"
...
def test_drop_micro_states():
    ...
    assert (
        output == expected_output
    ), f"Dropping micro states is incorrect. Expected: {expected_output}, Got: {output}"


def test_purify_macro_states():
    states = {
        "PerSWS": [[10, 20], [30, 40], [50, 60]],
        "PerREM": [[20, 30], [40, 50]],
        "PerMicroA": [[15, 17], [35, 37]],
        "PerHVS": [[55, 57]],
    }
    ...
    expected_output = {
        "PerSWS": [[10, 15], [17, 20], [30, 35], [37, 40], [50, 55], [57, 60]],
        "PerREM": [[20, 30], [40, 50]],
        "PerMicroA": [[15, 17], [35, 37]],
        "PerHVS": [[55, 57]],
    }
    assert (
        output == expected_output
    ), f"Purifying macro states is incorrect. Expected: {expected_output}, Got: {output}"
