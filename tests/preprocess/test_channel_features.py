...
def xwin(xsig):
    ...
    return rolling_win(xsig, window_size=512, window_step=64, dim='time').chunk({'AP':-1, 'ML':-1, 'window': -1, 'time': 3})
...
# ---------- temporal features ----------
TEMP_FEATURES = [
    ("noise_to_signal",      lambda x, **k: ft.noise_to_signal(x.data, fs=k["fs"]), (16,16,7)),
...
@pytest.mark.parametrize("name,fn,expected", TEMP_FEATURES, ids=[n for n,_,_ in TEMP_FEATURES])
...
# ---------- spatial features ----------
SPATIAL_FEATURES = [
    ("anticorrelation",            lambda x, n: ft.anticorrelation(x.isel(time=0).data, n.adj),              (16,16)),
    # ("laplacian",                  lambda x, n: ft.laplacian(x.isel(time=0), n.grid_shape),                  (16,16,512)),
    ("temporal_mean_laplacian",    lambda x, n: ft.temporal_mean_laplacian(x.isel(time=0)),                  (16,16)),
    # ("spatial_gradient",           lambda x, n: ft.spatial_gradient(x.isel(time=0)),                         (16,16)),
    ("gradient_rms_fast",          lambda x, n: ft.gradient_rms_fast(x.isel(time=0).data, n.adj_src, n.adj_dst),(16,16)),
]

@pytest.mark.parametrize("name,fn,expected", SPATIAL_FEATURES, ids=[n for n,_,_ in SPATIAL_FEATURES])
