...
def test_multitaper_spectrogram():    
	...
	np.random.seed(42)
	xsig = xr.DataArray(np.random.randn(2500), dims=('time'), coords={'time': np.arange(2500) / fs})
	...
	xwin = rolling_win(xsig, window_size=window_size, window_step=window_step, dim='time', window_dim='window')
	mtx = xr.apply_ufunc(
		sp.multitaper_psd,
		xwin,
	...
	freqs, times, scispec_spectrogram = scispec(xsig, fs=fs, nperseg=window_size, noverlap=window_size-window_step)
	# Assert the spectrogram results are close
	np.allclose(mtx, scispec_spectrogram.T, atol=5e-2)

	print("Spectrogram comparison passed, the results are consistent with scipy.signal spectrogram.")

def test_multitaper_rolling_window():
	xsig = ld.load_sample()
	xwin = rolling_win(xsig, window_size=128, window_step=64, dim='time').chunk({'AP':-1, 'ML':-1, 'window': -1})
	...
	assert np.allclose(xspec_dask_ufunc.chunk({"freq": -1, "time": 3}).compute(), xspec_dask_ufunc.compute())
...
def test_multitaper_fft():
	...
	psd_rec = sp.specx_coords(psd_rec, nfft=128, fs=xsig.fs, freq_dim='freq', time_dim='time')
	xspec = sp.multitaper_spectrogram(xsig, window_size=128, window_step=64).compute()
	assert [np.allclose(((np.abs(sp.multitaper_fft(xwin.isel(time=iwin)))**2)/128).mean(axis=-2), sp.multitaper_psd(xwin.isel(time=iwin))) for iwin in range(xwin.sizes['time'])]
...
def test_multitaper_fftgram():
	...
	assert ((np.abs(mt_fft_running.compute()) ** 2 / N).mean('taper') == mt_specgram.compute()).all()
