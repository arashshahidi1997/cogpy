...
def test_rolling_win_simple():
    ...
    xwin = sl.rolling_win(
        xsig, window_size=3, window_step=2, dim="time", window_dim="window"
    )
    ...
    assert np.array_equal(xwin.values, np.array([[1.0, 2.0, 3.0]]))
...
def test_rolling_win_eager():
    ...
    N = 11
    xsig = xr.DataArray(
        np.arange(N), dims=["time"], coords={"time": np.linspace(0, 1, N)}
    )
    xwin = _rolling_win_eager(
        xsig, window_size=3, window_step=1, dim="time", window_dim="window"
    )
    expected = np.array(
        [
            [0.0, 1.0, 2.0],
...
@pytest.mark.parametrize(
    "N, window_size, window_step",
    product(
        [5, 10, 11, 50], [3, 4, 5, 7], [1, 2, 3, 4]  # N  # window_size  # window_step
    ),
)
...
def test_running_measure():
    def running_corrcoef(
        ...
        x_roll = sl.rolling_win(
            xsig,
            window_size=window_size_samples,
            window_step=window_step_samples,
            dim=run_dim,
        )
    ...
    run_corrcoef1 = running_corrcoef(
        xsig,
        fs=xsig.fs,
        window_size=1 * pq.s,
        window_step=0.1 * pq.s,
        run_dim="time",
        corrcoef_dim="ch",
    )
    ...
    run_corrcoef = sl.running_measure(
        ...
        fs=fs,
        slider_kwargs=dict(
            window_size=seconds_to_samples(1 * pq.s, fs),
            window_step=seconds_to_samples(0.1 * pq.s, fs),
        ),
    ...
    assert np.allclose(
        run_corrcoef1, run_corrcoef
    ), "sl.running_measure(np.corrcoef) mismatch."
