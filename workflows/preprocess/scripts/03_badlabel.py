...
def compute_badscore(feature_ds):
	featurex = feature_ds.to_array('feature').stack(ch=['AP', 'ML']).chunk({'feature':-1}).reset_index('ch')
	badscorex = xr.apply_ufunc(deviation_zscore, featurex, input_core_dims=[['ch', 'feature']], output_core_dims=[['ch']], vectorize=True, dask='parallelized', output_dtypes=[featurex.dtype])
	return badscorex
